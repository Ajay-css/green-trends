          ...
          <div className="flex sm:flex-col overflow-x-auto sm:overflow-y-scroll justify-between sm:justify-normal sm:w-[18.7%] w-full">
            {productData.image.map((item, index) => (
              <img
                ...
                key={index}
                alt=""
                className="w-36 sm:mb-3 flex-shrink-0 cursor-pointer"
              />
            ))}
          </div>
        ...
        <div className="flex-1">
            ...
            <div className="flex gap-2">
              {productData.sizes.map((item, index) => (
                <button
                  ...
                  className={`border py-2 px-4 bg-gray-100 ${item.size === size?.size ? 'border-orange-500' : ''}`}
                  key={index}
                >
                  {item.size}
                </button>
          ...
          <button
            onClick={() => {
              ...
              !token ? toast.error('Please Login to Continue') : ''
            }
            } // ✅ only size string
